# Veganashi — Perfil de Cliente

## Información General

- **Empresa:** Veganashi
- **Sector:** Servicios — alimentación vegana / healthy food
- **Web:** https://veganashi.es
- **Prioridad:** Alta
- **Meta Ad Account ID:** act_928041200992402 *(secretos en .env — no hardcodear)*
- **Google Ads account_id:** `512-424-2748`
- **Facturación agencia:** tarifa fija mensual

## Negocio

### Qué hace
Empresa de servicios en el sector de alimentación vegana/saludable.

### Producto / Servicio principal
<!-- PENDIENTE — completar con cliente -->

### Propuesta de valor única
<!-- PENDIENTE -->

### Ticket medio
<!-- PENDIENTE -->

### Ciclo de venta
Servicios — proceso de decisión medio/corto.

## Audiencia

### Cliente ideal (ICP)
<!-- PENDIENTE — completar con datos reales de las campañas activas -->
- **Ubicación:** <!-- PENDIENTE -->
- **Intereses:** alimentación saludable, veganismo, estilo de vida activo
- **Pain points:** <!-- PENDIENTE -->

## Objetivos de Marketing

### Objetivo principal
**Leads** (formularios, contactos) + **notoriedad de marca**
No se priorizan ventas directas.

### KPIs objetivo
- **CPL objetivo:** a definir — conversiones no configuradas correctamente (bloqueante)
- **Volumen leads/mes objetivo:** subir leads entre semana + llenar local los fines de semana
- **Presupuesto mensual total (Meta + Google):** €600
  - Split: €400 Meta (67%) / €200 Google (33%)
  - Meta desglose: €300 prospecting / €100 retargeting
- **Campaña:** continua (sin fecha de fin)

## Plataformas Activas

| Plataforma | Activo | Budget mensual | Objetivo |
|------------|--------|---------------|---------|
| Meta Ads | ✅ | ~€400 | Leads + Awareness (llenar local fds) |
| Google Ads | ✅ | ~€200 | Búsqueda de intención (leads semana) |
| SEO | ☐ | — | |

## Creativos y Marca

### Assets disponibles
- ✅ Fotos de producto (proporcionadas por cliente)
- ✅ Vídeos editados (los genera un creador de contenido propio del cliente)
- ☐ Logo alta resolución — confirmar
- ☐ Brand guidelines — confirmar

### Flujo de creativos
El cliente tiene creador de contenido propio que entrega vídeos ya editados.
Imágenes estáticas se generan con Nano Banana o Canva.

### Tono de voz
<!-- PENDIENTE -->

## Competencia

<!-- PENDIENTE — usar Ahrefs para identificar competidores -->

## Historial y Notas

- Todos los objetivos son de leads/awareness — NO ecommerce ni ventas directas
- Presupuesto Meta activo ~€1.000/mes
- El cliente recibe reporte mensual con estilo de su marca

## Bloqueantes activos
- ⚠️ **Conversiones no configuradas correctamente** — sin esto no se puede calcular CPL real ni optimizar correctamente
- Prioridad: configurar eventos de conversión antes de escalar budget

## Última actualización
2026-06-03
