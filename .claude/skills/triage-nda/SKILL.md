...
- **Sin delegación formal**: en RD firma Kevin. GREEN = listo para firma tras su lectura rápida; YELLOW/RED = enviar al abogado con el resumen de issues de esta skill
- **Guardado**: `clients/<cliente>/contracts/YYYY-MM-DD_nda-triage.md`
- **Contexto España/UE**: revisar también cláusulas de protección de datos (RGPD) incrustadas en el NDA — si hay tratamiento de datos personales, hace falta DPA aparte, no basta el NDA
- **Si el NDA viene de un prospecto**: reflejar el estado del triage en la tarea Notion del deal
