...
- **Sistema existente**: los competidores de RD/Agencia Agéntica tienen dossier vivo en `agencia/inteligencia-competitiva/<competidor>/` (ej. Vibiz) con protocolo de captura propio — esta skill alimenta esos dossiers; las decisiones derivadas van primero a `decisiones-roadmap.md` y solo después suben a `AGENCIA-AGENTICA.md` como F-###
- **Regla de oro**: nunca usar al competidor como vendor con datos de clientes de RD
- **Briefs para clientes**: análisis competitivo de un cliente → `clients/<nombre>/reports/YYYY-MM-DD_competitive-brief.md`
- **Herramientas**: Ahrefs MCP para gaps de contenido, keywords y backlinks del competidor
