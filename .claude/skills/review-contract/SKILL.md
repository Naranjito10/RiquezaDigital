...
- **Playbook**: RD aún no tiene playbook formal de negociación. Usa como precedente los contratos de `clients/proponai/contracts/` (CEREBELLUS v2.x): mantenimiento con facturación anual y precios de referencia, SLAs por niveles, DPA limitado a datos comerciales/operacionales, límites de responsabilidad y modelo de precio cerrado. Cuando una posición se repita en 2+ contratos, propón consolidarla como playbook en `shared/sops/`.
- **Guardado**: contratos y revisiones en `clients/<cliente>/contracts/YYYY-MM-DD_<tipo>.md`
- **Jurisdicción por defecto**: España/UE — vigilar RGPD/LOPDGDD y cláusulas de encargado de tratamiento (art. 28 RGPD)
- **Checkpoint obligatorio**: la versión final siempre pasa por el abogado antes de enviarse al cliente — esta skill prepara la negociación, no sustituye asesoría legal
