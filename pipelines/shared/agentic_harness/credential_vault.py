...
class CredentialVault:
    ...
    leídas dinámicamente de .env, aislando las credenciales del código generado.
    Usa la librería estándar urllib para evitar dependencias externas.
    """
    @staticmethod
    def _load_env_key(key_name: str) -> str:
        # 1. Intentar leer del entorno del sistema
        val = os.environ.get(key_name)
        if val:
            return val
            
        # 2. Leer del archivo .env en la raíz del proyecto
    ...
    def _request_gemini(cls, model: str, system_prompt: str, prompt: str, json_mode: bool) -> str:
        ...
        if not api_key:
            raise ValueError("GEMINI_API_KEY no encontrada en el archivo .env ni en variables de entorno.")
    ...
    def _request_anthropic(cls, model: str, system_prompt: str, prompt: str, json_mode: bool) -> str:
        ...
        if not api_key:
            raise ValueError("ANTHROPIC_API_KEY no encontrada en el archivo .env ni en variables de entorno.")
